 ...
 * @ingroup themeable
 */
function yellow_menu_local_task($mid, $active, $primary) {
  $active_class = "";
  if ($active) {
    $active_class .= "active ";
  }
  $link = menu_item_link($mid, FALSE);
  $output = '<span class="'.$active_class.'yellow-button-wrapper">'.
    '<span class="yellow-button-l"></span>'.
    '<span class="yellow-button-r"></span>'.
    '<a href="?q='.$link['href'].'" class="'.$active_class.'yellow-button">'.$link['title'].'</a></span>';
  return '<li>'.$output.'</li>';
}
 ...
 *   The url of the feed.
 */
function yellow_feed_icon($url) {
  return '<a href="'. check_url($url) .'" class="yellow-rss-tag-icon" title="' . t('Syndicate content') . '"></a>';
}

/**
 * Allow themable wrapping of all comments.
 */
function yellow_comment_wrapper($content, $type = null) {
  static $node_type;
...
<div class="yellow-box yellow-post">
  ...
  if ($content && $node_type != 'forum') {
    $result .= '<h2 class="yellow-postheader comments">' . t('Comments') . '</h2>';
  }
 ...
 */
function yellow_breadcrumb($breadcrumb) {
  if (!empty($breadcrumb)) {
    return '<div class="breadcrumb yellow-postcontent">'. implode(' | ', $breadcrumb) .'</div>';
  }
}

function yellow_service_links_node_format($links) {
 ...
 * @ingroup themeable
 */
function yellow_button($element) {
  // Make sure not to overwrite classes.
  if (isset($element['#attributes']['class'])) {
    $element['#attributes']['class'] = 'form-'.$element['#button_type'].' '.$element['#attributes']['class'].' yellow-button';
  }
  else {
...
*/
function yellow_img_assist_page($content, $attributes = NULL) {
