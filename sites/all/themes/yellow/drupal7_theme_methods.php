...
function yellow_process_html(&$variables) {
...
function yellow_breadcrumb($variables) {
  $breadcrumb = $variables['breadcrumb'];

  if (!empty($breadcrumb)) {
    // Provide a navigational heading to give context for breadcrumb links to
    // screen-reader users. Make the heading invisible with .element-invisible.
    $output = '<h2 class="element-invisible yellow-postheader">' . t('You are here') . '</h2>';

    $output .= '<div class="breadcrumb yellow-postcontent">' . implode(' » ', $breadcrumb) . '</div>';
    return $output;
  }
}
 ...
 * @ingroup themeable
 */
function yellow_button($variables) {
  $element = $variables['element'];
  $element['#attributes']['type'] = 'submit';
  element_set_attributes($element, array('id', 'name', 'value'));

  $element['#attributes']['class'][] = 'form-' . $element['#button_type'] . ' yellow-button';
  if (!empty($element['#attributes']['disabled'])) {
    $element['#attributes']['class'][] = 'form-button-disabled';
  }

  return '<span class="yellow-button-wrapper">'.
    '<span class="yellow-button-l"></span>'.
    '<span class="yellow-button-r"></span>'.
 ...
 */
function yellow_preprocess_page(&$vars) {
 ...
 * @ingroup themeable
 */
function yellow_menu_local_task($variables) {
  ...
  //added yellow-class
  $link['localized_options']['attributes']['class'] = array('yellow-button');

  return '<li>' .
	  '<span class="yellow-button-wrapper' . $active_class. '">'.
	  '<span class="yellow-button-l"></span>'.
      '<span class="yellow-button-r"></span>'.
 ...
 */
function yellow_feed_icon($variables) {
  $text = t('Subscribe to @feed-title', array('@feed-title' => $variables['title']));
  return l(NULL, $variables['url'], array('html' => TRUE, 'attributes' => array('class' => array('feed-icon', 'yellow-rss-tag-icon'), 'title' => $text)));
}
 ...
 * @ingroup themeable
 */
function yellow_node_preview($variables) {
  $node = $variables['node'];
...
$terms = get_terms_D7($content);
  if (!empty($terms)) {
  ob_start();?>
  <span class="yellow-posttagicon"><?php
  $result .= ($result == '') ? ob_get_clean() : '&nbsp;|&nbsp;' . ob_get_clean();
  $result .= '<div class="yellow-tags">' . render($terms) . '</div>';
  ob_start();?>
  </span><?php
  $result .= ob_get_clean();
  }
...
if (strpos ($key, "comment") !== FALSE) {
		...
		ob_start();?>
		<span class="yellow-postcommentsicon"><?php
		$output .= ob_get_clean();
		$output .= $link_output;
		$index++;
		ob_start();?>
...
if ($index > 0 && !empty($link_output) && !empty($output)) {
        ...
        ob_start();?>
        <span class="yellow-postcategoryicon"><?php
        $output .= ob_get_clean();
        $output .= $link_output;
        $index++;
        ob_start();?>
