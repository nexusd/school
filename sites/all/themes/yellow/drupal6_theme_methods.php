...
function yellow_menu_local_task($link, $active = FALSE) {
  $active_class = "";
  if ($active) {
    $active_class .= "active ";
  }
  $output = preg_replace('~<a href="([^"]*)"[^>]*>([^<]*)</a>~',
    '<span class="'.$active_class.'yellow-button-wrapper">'.
    '<span class="yellow-button-l"></span>'.
    '<span class="yellow-button-r"></span>'.
    '<a href="$1" class="'.$active_class.'yellow-button">$2</a></span>', $link);
  return '<li>'.$output.'</li>';
}
 ...
 *   A descriptive title of the feed.
  */
function yellow_feed_icon($url, $title) {
  return '<a href="'. check_url($url) .'" class="yellow-rss-tag-icon" title="' . $title . '"></a>';
}

/**
 * Add a "Comments" heading above comments except on forum pages.
 */
function yellow_preprocess_comment_wrapper(&$vars) {
 ...
 * @ingroup themeable
 */
function yellow_node_preview($node) {
